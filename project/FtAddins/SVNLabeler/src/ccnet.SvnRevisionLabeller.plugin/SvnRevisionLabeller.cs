...
namespace CcNet.Labeller
{
    ...
    {
        private static Regex _extractVersionInfoRegex = new Regex(@"\[assembly\: (AssemblyVersionAttribute|AssemblyVersion)\(""(\d{1,})\.(\d{1,})\.(\d{1,})\.(\d{1,})""\)\]", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
        ...
        /// <summary>
        /// Tells labeler to only retrive major, minor and build versions from assembly.cs 
        /// </summary>
        [ReflectorProperty("getVersionInfoFromAttribute", Required = false)]
        public bool GetVersionInfoAssemblyCS { get; set; }

        /// <summary>
        /// 
        /// </summary>
        [ReflectorProperty("pathToAssemblyCS", Required = false)]
        public string PathToAssemblyCS { get; set; }


        [ReflectorProperty("writeBackVersionToAsseblyCS", Required = false)]
        public bool WriteBackVersionToAsseblyCS { get; set; }
        ...
        public void Run(IIntegrationResult result)
        {
            ...
            result.IntegrationProperties.Add("CCNetMajor", Major.ToString());
            result.IntegrationProperties.Add("CCNetMinor", Minor.ToString());
            result.IntegrationProperties.Add("CCNetBuild", Build.ToString());
            result.IntegrationProperties.Add("CCNetRevision", _revision.ToString());       

           
            result.Parameters.Add(new NameValuePair("${CCNetMajor}", Major.ToString()));
            result.Parameters.Add(new NameValuePair("${CCNetMinor}", Minor.ToString()));
            result.Parameters.Add(new NameValuePair("${CCNetBuild}", Build.ToString()));
            result.Parameters.Add(new NameValuePair("${CCNetRevision}", _revision.ToString()));


            Environment.SetEnvironmentVariable("CCNetRevision", _revision.ToString(), EnvironmentVariableTarget.Process);
        ...
        public string Generate(IIntegrationResult resultFromLastBuild)
        {
            ...
            if (GetVersionInfoAssemblyCS)
            {
                PupulateVersionInfoFromAssemblyCS(resultFromLastBuild);
            ...
            string format = Pattern.Replace(MajorToken, "{0}")
                ...
                .Replace(TimeStampToken, "{7}");

            return String.Format(format, Major, Minor, build, revision, _rebuild, elapsedDays, msRevision, _systemClock.Now.ToString("yyyy-MM-dd HH-mm-ss"));
        }

        private void PupulateVersionInfoFromAssemblyCS(IIntegrationResult resultFromLastBuild)
        {
            try
            {
                if (!string.IsNullOrEmpty(PathToAssemblyCS))
                {
                    string pathToFile = string.Empty;

                    if (Path.IsPathRooted(PathToAssemblyCS))
                    {
                        pathToFile = PathToAssemblyCS;

                    }
                    else
                    {
                        pathToFile = resultFromLastBuild.WorkingDirectory.TrimEnd('\\') + "\\" + pathToFile.TrimStart('\\');
                    }
                    ...
                    if (File.Exists(pathToFile))
                    {
                        ...
                        var match = _extractVersionInfoRegex.Match(fileContents);

                        this.Major = int.Parse(match.Groups[1].Value);

                        this.Minor = int.Parse(match.Groups[2].Value);

                        this.Build = int.Parse(match.Groups[3].Value);


                        if (WriteBackVersionToAsseblyCS)
                        {

                            fileContents = _extractVersionInfoRegex.Replace(fileContents, string.Format("[assembly: AssemblyVersionAttribute(\"{0}.{1}.{2}.{3}\")]", Major, Minor, Build, _revision));

                            File.WriteAllText(pathToFile, fileContents);

                        }
                    }
                    else
                    {
                        Log.Error("{0} could not locate assembly.cs file at {1}", typeof(SvnRevisionLabeller), pathToFile);
